...
def mutational_scan_note(settings: dict) -> str:
    parents = resolve_binder_sequences(settings)
    cap = int(settings.get('max_mutations_per_sequence', 0) or 0)
    return f"mutational scan: {len(parents)} given sequence(s) of {'/'.join(str(len(sequence)) for sequence in parents.values())} residue(s), {f'at most {cap} substitution(s)' if cap else 'an uncapped walk'} from the parent"
...
CAMPAIGN_FEATURES = (
    ...
    CampaignFeature('mutational scan', lambda settings: bool(settings.get('binder_sequences')), shorthand='scan', switch='binder_sequences',
                    checks=(ModalityCheck('Binder_Mutations', 0.0, True, 'min_binder_mutations_final', parameters=mutation_check_parameters),),
                    note=mutational_scan_note,
                    conflicts=(('fold conditioning', 'a mutational scan starts from a sequence that is given, so there is no scaffold to rebuild'),)),
...
def resolve_binder_sequences(settings: dict) -> dict[str, str]:
    ...
    if not isinstance(binder_sequences, dict):
        raise ValueError('binder_sequences is written {"name": "SEQUENCE"}, naming every parent the scan starts from')
    resolved = {}
